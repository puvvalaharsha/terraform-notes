...
resource "aws_key_pair" "mykeypair" {
  key_name   = "keypair-custome"  # Replace with your desired key name
...
resource "aws_instance" "server" {
 ...
 provisioner "local-exec" {
  ...
  provisioner "file" {
    source      = "fil1"  # Replace with the path to your local file
    destination = "/home/ec2-user/fil1"  # Replace with the path on the remote instance
  }
  # remote execution process 
  provisioner "remote-exec" {
    inline = [
"touch file4",
"echo hello from aws >> file4",
]
 }
}
